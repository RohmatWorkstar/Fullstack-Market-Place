...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="container-custom max-w-4xl py-16 animate-fade-in">
        <h1 className="text-3xl font-bold text-surface-900 dark:text-surface-50 mb-8">{tCart.title}</h1>
        <div className="glass rounded-3xl p-12 text-center text-surface-500 dark:text-surface-300">
          <ShoppingBag className="w-16 h-16 mx-auto mb-6 text-surface-300 dark:text-surface-600" />
          <h2 className="text-xl font-semibold text-surface-900 dark:text-surface-50 mb-2">{tCart.emptyTitle}</h2>
  ...
  return (
    <div className="container-custom max-w-6xl py-8 animate-fade-in">
      <Link href="/products" className="inline-flex items-center gap-2 text-sm text-surface-500 dark:text-surface-300 hover:text-surface-900 dark:hover:text-surface-50 mb-6 transition-colors">
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-4">
          <div className="glass rounded-3xl overflow-hidden hidden sm:block">
            <div className="grid grid-cols-12 gap-4 border-b border-surface-200 dark:border-surface-800 px-6 py-4 bg-surface-50/50 dark:bg-surface-800/80">
              <div className="col-span-6 font-medium text-surface-600 dark:text-surface-200 text-sm uppercase tracking-wider">{tCart.product}</div>
              <div className="col-span-3 font-medium text-surface-600 dark:text-surface-200 text-sm uppercase tracking-wider text-center">{tCart.quantity}</div>
              <div className="col-span-3 font-medium text-surface-600 dark:text-surface-200 text-sm uppercase tracking-wider text-right">{tCart.total}</div>
            </div>
          </div>
              ...
              <div key={item.product.id} className="glass rounded-3xl p-4 sm:px-6 relative group overflow-hidden">
                <div className="flex flex-col sm:grid sm:grid-cols-12 gap-6 items-center">
                   ...
                   <div className="w-full sm:col-span-6 flex items-center gap-4 border-b border-surface-200 pb-4 sm:border-0 sm:pb-0">
                     <Link href={`/products/${item.product.id}`} className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-surface-100 dark:bg-surface-800 flex-shrink-0 border border-surface-200 dark:border-surface-700 overflow-hidden block">
                     ...
                     <div className="flex-1 min-w-0 pr-6">
                        <Link href={`/products/${item.product.id}`} className="text-base sm:text-lg font-semibold text-surface-900 dark:text-surface-50 hover:text-primary-600 transition-colors line-clamp-2 leading-tight mb-2">
                          {item.product.name}
                        </Link>
                        <p className="text-primary-600 font-medium sm:hidden mb-1">{formatPrice(item.product.price)}</p>
                        <p className="text-sm text-surface-500 dark:text-surface-300 sm:hidden">Max: {item.product.stock}</p>
                     </div>
                   </div>
                   ...
                   <div className="w-full sm:col-span-3 flex justify-between sm:justify-end items-center">
                     <span className="text-sm text-surface-500 dark:text-surface-400 sm:hidden">{tCart.total}:</span>
                     <div className="font-bold text-surface-900 dark:text-surface-100 sm:text-lg">
        ...
        <div className="glass rounded-3xl p-6 lg:p-8 sticky top-24 shadow-sm border-t-4 border-t-primary-500">
          <h2 className="text-xl font-bold text-surface-900 dark:text-surface-50 mb-6">{tCart.orderSummary}</h2>
          ...
          <div className="border-t border-surface-200 dark:border-surface-800 pt-6 mb-8">
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold text-surface-900 dark:text-surface-100 text-lg">Total</span>
              <span className="font-bold text-primary-600 text-2xl">{formatPrice(subtotal)}</span>
            </div>
            <p className="text-xs text-surface-500 dark:text-surface-400 text-right">Includes taxes</p>
          </div>
